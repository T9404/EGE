...
# переворачиваем массив, в некоторых задачах требуется!
arr = arr[::-1]  
print(arr.count(6))
...
def f(num, cc):
    ...
    # спросили про первую цифру, а вы не повернули число? минус балл
    num_cc = num_cc[::-1]  
    return num_cc


number = f(2 ** 1024 + 2 ** 1025, 16)
print(number[0])
...
for num_2 in range(3, 10):
    if (1 * num_2 ** 0) + (2 * num_2 ** 1) + (1 * num_2 ** 2) + 1 == (1 * 9 ** 0) +  \
        (0 * 9 ** 1) + (1 * 9 ** 2):             
...
# форматирование строки. на выходе строка, 140:b, b - bin
print(f'{140:b}') 
print(f'{140:o}')
print(f'{140:h}') 
