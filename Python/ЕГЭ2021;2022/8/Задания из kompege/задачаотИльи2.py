...
s = sorted(set([x for x in map(lambda x: ''.join(x), product(al, repeat=7)) if all(
    x[0] != f for f in sogl) and all('ё'+l not in x for l in sogl) and all(l+'ё' not in x for l in sogl)]))
