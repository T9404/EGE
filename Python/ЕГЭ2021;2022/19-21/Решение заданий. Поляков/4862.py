...
def move(e):
    ...
    # flag - первая карта? усл: "Первая карта может быть любой из доступных."
    ...
    if not flag: 
        ...
        new_h.append((*m, l, s, True))

        return new_h # убираем самую первую карту
...
arr19 = [i for i in set(cards_19) if f((*cards_19, i, i, False))[0] == 'P']
print(*arr19)
...
arr20 = [i for i in set(cards_20) if f((*cards_20, i, i, False))[0] == 'P']
print(min(arr20), max(arr20))




# массив с картами для набора
...
for i, j, m, u in product(range(4), repeat=4):
    ...
    if all(f((*q, i, i, False))[0] == 'V' for i in range(4, 7+1)): 
        answer_21 += 1
